...
type codeColor = Red | Green | Yellow | Purple | White | Black | Placeholder
...
let validateCode (tryCode : code) (trueCode : code) =
  let mutable (white: int) = 0
  let mutable (black: int) = 0
  let mutable (temporaryCode: code) = trueCode
  for i = 0 to (tryCode.Length - 1) do
      if tryCode.[i] = trueCode.[i] then
          black <- black + 1
          printfn "%A"temporaryCode
          temporaryCode <- listChange i temporaryCode
          printfn "%A"temporaryCode
  for i = 0 to (tryCode.Length - 1) do
      if (List.contains tryCode.[i] temporaryCode) then
          white <- white + 1
  (white, black)
/////////////////////////////////////////////////////////////////////////////////////////////
///                                 Play Game                                             ///
...
let rec playGame guess =
  Console.Clear()
  let (Val: answer) = (validateCode theCode theGuess) /// <remarks> Defines the answer, to be added to the board. </remarks>
