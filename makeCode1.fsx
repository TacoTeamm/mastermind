...
let rec validateCode (tryCode : code) (trueCode : code) (white: int) (black: int) =
  ...
  match tryCode with
  | [] -> (white, black)
  | x::xs when x = temporaryCode.[(4 - (tryCode.Length))] -> (validateCode xs (listChange (4 - (tryCode.Length)) temporaryCode) white (black + 1))
  | x::xs when (List.contains x temporaryCode) -> (validateCode xs temporaryCode (white + 1) black)
  | x::xs -> (validateCode xs temporaryCode white black)
