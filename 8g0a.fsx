...
let randomizer (x : int) =
    let mutable (colorPossibilities : code) = codeColorList
    let mutable (sample : code) = []
    let rand = Random()
    let mutable (imax : int) = colorPossibilities.Length
...
let rec validateCode (tryCode : code) (trueCode : code) (white: int) (black: int) =
  match tryCode with
  | [] -> (white, black)
  | x::xs when x = trueCode.[(guessLength - (tryCode.Length))] -> (validateCode xs trueCode white (black + 1))
